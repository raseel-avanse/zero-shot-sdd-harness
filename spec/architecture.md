...
Sentinel is a single-origin web application: a Next.js static-export dashboard (served at `/app`) talking to a FastAPI backend on `http://localhost:8001`, which drives a LangGraph agent that performs whitebox security assessments against a scope-approved local source repository using the Gemini API, persisting engagements, scope records, findings, and per-run token/cost in PostgreSQL. A security-team user creates a scope-gated engagement, starts an assessment run, and watches validated finding cards stream in over SSE.

## Component Map

```
[Next.js static UI @ /app]
        │  fetch + EventSource (same origin :8001)
        ▼
...
- **Backend:** FastAPI (uvicorn, `api:app`, port 8001); run via `uv run python -m src`
...
## Deployment Model
Local long-running service: `uv run python -m src` serves FastAPI + the static UI under `/app` on port 8001; PostgreSQL runs in Docker (`sec-agent-pg`, host :5433). Single origin, single user (localhost) for the MVP.
