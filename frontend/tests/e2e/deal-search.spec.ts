...
test('product-name search returns a ranked deal list with a cost footer', async ({ page }) => {
  ...
  // The three real search modes are present (no more "Coming soon" stubs).
  await expect(page.getByTestId('mode-tabs')).toBeVisible()
  await expect(page.getByTestId('mode-name')).toBeEnabled()
  await expect(page.getByTestId('mode-url')).toBeEnabled()
  await expect(page.getByTestId('mode-category')).toBeEnabled()
  ...
  await expect(page.getByTestId('progress-label')).toContainText(
    /Searching|Reading|Ranking|Checking/,
  )
  ...
  if (count === 0) {
    ...
    // P2: every card renders a real deal-quality badge (no disabled stub).
